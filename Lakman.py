...
def only_char_recur(string):
    # Рекурсивная проверка уникальности символов в строке, время выполнения О(n**2)
    if len(string) == 1:
        return True
...
class MyLL:
    ...
    def numbers_reverse(self):
        # Вывод целого числа, сострящим из всех значений списка в обратном порядке
    ...
    def doubles_delete_sorted(self):
        # Удаление дубликатов из сортированного списка, время выполнения O(k), где k - размер списка
        if self.length > 1:
            curr = self.first
            while curr.next_node:
...
def bracket_combinations(amount):
    # Вывод всех правильных скобочных последовательностей из amount пар скобок
    if amount == 1:
        return {'()'}
    res = set()
...
def b_search(array, x, i=0):
    # Бинарный поиск индекса заданного элемента в сортированном списке со сдвигом
    n = len(array)
    if n < 2:
        return i
    ...
    if array[0] < array[n]:
        if x < array[0]:
            return b_search(array[n:], x, i + n)
        elif x < array[n]:
            return b_search(array[:n], x, i)
        elif x == array[0]:
            return i
        else:
            return i + n
...
def nuggets(boxes: list, amount: int or float, result=None,
            temp=None, answers=None):
    ...
    if len(boxes) == 1:
        if temp >= boxes[0]:
            ...
            nuggets(boxes, amount, result=result.copy(),
                    temp=temp-boxes[0], answers=answers)
        return answers
    if temp >= boxes[0]:
        result.append(boxes[0])
        for i in range(len(boxes)):
            nuggets(boxes[i:], amount, result=result.copy(),
                    temp=temp-boxes[0], answers=answers)
    else:
        nuggets(boxes[1:], amount, result=result.copy(),
                temp=temp, answers=answers)
    nuggets(boxes[1:], amount, answers=answers)
    return answers
